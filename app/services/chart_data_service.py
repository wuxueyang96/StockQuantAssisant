...
def _decision_points(df_daily: pd.DataFrame, intraday: dict[str, pd.DataFrame], bars: int) -> list[dict]:
    ...
    for ts, row in decisions.tail(bars).iterrows():
        ...
        rows.append({
            ...
            'structure_effect': row.get('structure_effect'),
            'sequence_effect': row.get('sequence_effect'),
            'hard_exit': _json_bool(row.get('hard_exit')),
            'hard_exit_reason': row.get('hard_exit_reason'),
            'target_reversal_warning': row.get('target_reversal_warning'),
            'high9_active': _json_bool(row.get('high9_active')),
            'low9_active': _json_bool(row.get('low9_active')),
        })
    return rows
