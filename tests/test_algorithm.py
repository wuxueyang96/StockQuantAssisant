...
class TestTrendChannel:
    ...
    def test_short_upper_uses_ema_of_rolling_max_close(self, trend):
        """short_upper 默认使用 Close 的滚动最大值（channel_upper_source='close'）。

        收盘价比最高价更能反映价格的真实落脚点，避免极端影线虚胖通道。
        """
        np.random.seed(7)
        n = 200
        ...
        for i in range(0, n, 5):
            closes[i] = 110.0
        highs = closes + 2.0  # High 高于 Close，验证系统用的是 Close 而非 High
        lows = closes - 1.0
        opens = closes.copy()
        df = pd.DataFrame({
            'Open': opens, 'High': highs, 'Low': lows, 'Close': closes,
